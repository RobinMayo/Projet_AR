...
void initialization(void) {
	...
	sleep(1);
	MPI_Send(&var_to_find, 1, MPI_INT, searching_pair, searching_pair, MPI_COMM_WORLD);
	
	/* ----------- III. Gestion de la Dynamicite du Systeme : ----------- */
	deconnect_pair = (rand() % NB_PAIR) + 1;
	deconnect_suc = (deconnect_pair+1)% NB_PAIR;
	deconnect_pre = (deconnect_pair-1)% NB_PAIR;
	...
	printf("Peer %d is deconnected, succ is %d, resp is %d \n", deconnect_pair, i, j);
}

void pair(int rang) {
	int rang_succ, id, succ, resp;
	int key_to_find, deconnect, i;
	int searching_pair = 0;
	MPI_Status status;
	...
	if (key_to_find == -1) {
		...
		if(key_to_find <= id && key_to_find >= resp) {
			...
			MPI_Send(&key_to_find, 1, MPI_INT, rang_succ, searching_pair, MPI_COMM_WORLD);
		}
	}
	
	printf("Peer %d is waiting.\n", rang);
	MPI_Recv(&id, 1, MPI_INT, MPI_ANY_SOURCE, TAGSUCC, MPI_COMM_WORLD, &status);
	resp = deconnect;
